  ...
  it("plans a side-effect-free Kevin reset from the protected Teams peer", () => {
    ...
    expect(plan).toMatchObject({
      ...
      cleanup: {
        ...
        bwsSecret: {
          name: "employee_agent_kkilgo_bws_token_v1",
          command: "docker secret rm 'employee_agent_kkilgo_bws_token_v1'",
        },
        auth: {
          agentId: "kkilgo",
          provider: "openai",
    ...
    expect(plan.validation).toContain("employee BWS token secret is absent");
  });

...
export function createMSTeamsEmployeeOnboardingResetPlan(params: {
  ...
  const bwsSecretName = `employee_agent_${slug.replace(/-/gu, "_")}_bws_token_v1`;
  ...
  return {
    ...
    cleanup: {
      ...
      bwsSecret: {
        name: bwsSecretName,
        command: `docker secret rm ${shellSingleQuote(bwsSecretName)}`,
      },
      auth: {
        agentId,
        provider: "openai",
    ...
    validation: [
      ...
      "employee BWS token secret is absent",
